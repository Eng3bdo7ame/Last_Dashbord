...
const EditTaskForm = ({ card, onClose, onSave }) => {
    ...
    const handleClose = () => {
        setClosing(true);
        setTimeout(() => onClose(), 300); // Wait for the animation to finish before triggering onClose
    };
    ...
    return (
        <div className={`z-50 fixed top-0 right-0 w-full md:w-1/3 h-full bg-white shadow-lg rounded-lg transition-transform duration-300 ease-in-out ${closing ? 'translate-x-full' : 'translate-x-0'}`}>
            ...
            <div className="px-6 py-4 overflow-y-auto h-full" ref={formRef}>
